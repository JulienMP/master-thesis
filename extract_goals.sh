#!/bin/bash
#
#SBATCH --job-name=extract_goals_fixed
#SBATCH --output=/home/jmparirwa/master-thesis/extract_goals_fixed_log_%j.out
#SBATCH --error=/home/jmparirwa/master-thesis/extract_goals_fixed_error_%j.err
#
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=2
#SBATCH --mem=16G
#SBATCH --time=72:00:00   # 72 hours should be enough for processing the entire dataset
#
...
OUTPUT_DIR="${SCRATCH_DIR}/before_goal"
VENV_DIR="${HOME_DIR}/myenv"  # Path to your virtual environment
SCRIPT_PATH="${HOME_DIR}/extract_all_goals_fixed.py"
...
# Check if ffmpeg is installed on the system, if not install it
if ! command -v ffmpeg &> /dev/null; then
    echo "ffmpeg not found, installing..."
    # Different installation methods depending on the system
    if command -v apt-get &> /dev/null; then
        ...
        echo "Attempting to load ffmpeg module..."
...
# Set environment variable for OpenCV to avoid GUI dependencies
export OPENCV_VIDEOIO_MSMF_ENABLE_HW_TRANSFORMS=0

# Run the extraction script
echo "Starting goal clip extraction at $(date)"
echo "Data directory: ${DATA_DIR}"
echo "Output directory: ${OUTPUT_DIR}"

python ${SCRIPT_PATH} --data_dir ${DATA_DIR} --output_dir ${OUTPUT_DIR}

# Check the exit code
if [ $? -eq 0 ]; then
    echo "Goal clip extraction completed successfully at $(date)"
else
    echo "Goal clip extraction failed with exit code $? at $(date)"
fi
